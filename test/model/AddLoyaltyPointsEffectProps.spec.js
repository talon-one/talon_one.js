/**
 * Talon.One API
 * Use the Talon.One API to integrate with your application and to manage applications and campaigns:  - Use the operations in the [Integration API section](#integration-api) to integrate with our platform. - Use the operation in the [Management API section](#management-api) to manage applications and campaigns.  ## Determining the base URL of the endpoints  The API is available at the same hostname as your Campaign Manager deployment.  For example, if you access the Campaign Manager at `https://yourbaseurl.talon.one/`, the URL for the [updateCustomerSessionV2](https://docs.talon.one/integration-api#tag/Customer-sessions/operation/updateCustomerSessionV2) endpoint is `https://yourbaseurl.talon.one/v2/customer_sessions/{Id}`. 
 *
 * The version of the OpenAPI document: 
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD.
    define(['expect.js', process.cwd()+'/src/index'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    factory(require('expect.js'), require(process.cwd()+'/src/index'));
  } else {
    // Browser globals (root is window)
    factory(root.expect, root.TalonOne);
  }
}(this, function(expect, TalonOne) {
  'use strict';

  var instance;

  beforeEach(function() {
    instance = new TalonOne.AddLoyaltyPointsEffectProps();
  });

  var getProperty = function(object, getter, property) {
    // Use getter method if present; otherwise, get the property directly.
    if (typeof object[getter] === 'function')
      return object[getter]();
    else
      return object[property];
  }

  var setProperty = function(object, setter, property, value) {
    // Use setter method if present; otherwise, set the property directly.
    if (typeof object[setter] === 'function')
      object[setter](value);
    else
      object[property] = value;
  }

  describe('AddLoyaltyPointsEffectProps', function() {
    it('should create an instance of AddLoyaltyPointsEffectProps', function() {
      // uncomment below and update the code to test AddLoyaltyPointsEffectProps
      //var instane = new TalonOne.AddLoyaltyPointsEffectProps();
      //expect(instance).to.be.a(TalonOne.AddLoyaltyPointsEffectProps);
    });

    it('should have the property name (base name: "name")', function() {
      // uncomment below and update the code to test the property name
      //var instane = new TalonOne.AddLoyaltyPointsEffectProps();
      //expect(instance).to.be();
    });

    it('should have the property programId (base name: "programId")', function() {
      // uncomment below and update the code to test the property programId
      //var instane = new TalonOne.AddLoyaltyPointsEffectProps();
      //expect(instance).to.be();
    });

    it('should have the property subLedgerId (base name: "subLedgerId")', function() {
      // uncomment below and update the code to test the property subLedgerId
      //var instane = new TalonOne.AddLoyaltyPointsEffectProps();
      //expect(instance).to.be();
    });

    it('should have the property value (base name: "value")', function() {
      // uncomment below and update the code to test the property value
      //var instane = new TalonOne.AddLoyaltyPointsEffectProps();
      //expect(instance).to.be();
    });

    it('should have the property desiredValue (base name: "desiredValue")', function() {
      // uncomment below and update the code to test the property desiredValue
      //var instane = new TalonOne.AddLoyaltyPointsEffectProps();
      //expect(instance).to.be();
    });

    it('should have the property recipientIntegrationId (base name: "recipientIntegrationId")', function() {
      // uncomment below and update the code to test the property recipientIntegrationId
      //var instane = new TalonOne.AddLoyaltyPointsEffectProps();
      //expect(instance).to.be();
    });

    it('should have the property startDate (base name: "startDate")', function() {
      // uncomment below and update the code to test the property startDate
      //var instane = new TalonOne.AddLoyaltyPointsEffectProps();
      //expect(instance).to.be();
    });

    it('should have the property expiryDate (base name: "expiryDate")', function() {
      // uncomment below and update the code to test the property expiryDate
      //var instane = new TalonOne.AddLoyaltyPointsEffectProps();
      //expect(instance).to.be();
    });

    it('should have the property transactionUUID (base name: "transactionUUID")', function() {
      // uncomment below and update the code to test the property transactionUUID
      //var instane = new TalonOne.AddLoyaltyPointsEffectProps();
      //expect(instance).to.be();
    });

    it('should have the property cartItemPosition (base name: "cartItemPosition")', function() {
      // uncomment below and update the code to test the property cartItemPosition
      //var instane = new TalonOne.AddLoyaltyPointsEffectProps();
      //expect(instance).to.be();
    });

    it('should have the property cartItemSubPosition (base name: "cartItemSubPosition")', function() {
      // uncomment below and update the code to test the property cartItemSubPosition
      //var instane = new TalonOne.AddLoyaltyPointsEffectProps();
      //expect(instance).to.be();
    });

    it('should have the property cardIdentifier (base name: "cardIdentifier")', function() {
      // uncomment below and update the code to test the property cardIdentifier
      //var instane = new TalonOne.AddLoyaltyPointsEffectProps();
      //expect(instance).to.be();
    });

    it('should have the property bundleIndex (base name: "bundleIndex")', function() {
      // uncomment below and update the code to test the property bundleIndex
      //var instane = new TalonOne.AddLoyaltyPointsEffectProps();
      //expect(instance).to.be();
    });

    it('should have the property bundleName (base name: "bundleName")', function() {
      // uncomment below and update the code to test the property bundleName
      //var instane = new TalonOne.AddLoyaltyPointsEffectProps();
      //expect(instance).to.be();
    });

    it('should have the property awaitsActivation (base name: "awaitsActivation")', function() {
      // uncomment below and update the code to test the property awaitsActivation
      //var instane = new TalonOne.AddLoyaltyPointsEffectProps();
      //expect(instance).to.be();
    });

    it('should have the property validityDuration (base name: "validityDuration")', function() {
      // uncomment below and update the code to test the property validityDuration
      //var instane = new TalonOne.AddLoyaltyPointsEffectProps();
      //expect(instance).to.be();
    });

  });

}));
